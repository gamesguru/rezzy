...
\title{Matrix State Resolution V3: \\ A Surgical Fix for Invite Locks and Ban Evasion}
...
\section{State Resolution V3: The Surgical Merge}
State Resolution V3 addresses the vulnerabilities of both previous iterations by expanding the V2.1 supplemental merge to include \textbf{Authoritative Memberships}.
...
\node[event, fill=blue!10] (v3) {\textbf{V3 Auth Overlay}};
\node[event, right=of v3, fill=green!10] (pl) {\texttt{\$pl} (Included)};
\node[event, below=of pl, fill=green!10] (ban) {\texttt{\$ban} (Included)};
\node[event, below=of v3, fill=red!10] (invite) {\texttt{\$admin\_lock} (Excluded)};

\draw[->, thick] (v3) -- (pl);
\draw[->, thick] (v3) |- (ban);
\draw[->, thick, dotted] (v3) -- (invite) node[midway, sloped, above] {Isolated};

\end{tikzpicture}
\caption{State Resolution V3: The mathematical sweet spot. Power levels and Authoritative Memberships (Bans) are merged into the overlay to defeat evasion attacks, while \texttt{join\_rules} are cleanly isolated to preserve historical DAG integrity.}
\label{fig:v3_merge}
\end{figure}

\section{Defeating Demotion Evasion}
A separate vector, Demotion Evasion, occurs when a demoted administrator attempts to bypass their demotion by referencing an ancient Power Level event in their 1-hop \texttt{auth\_events} chain. Both V2.1 and V3 inherently defeat this attack because the authoritative demotion is a Power Level event, which is supplemented into the auth overlay and immediately invalidates the malicious event's credentials.
...
\end{tikzpicture}
\caption{Demotion Evasion: Eve maliciously points her event's \texttt{auth\_events} directly to her ancient PL 100 event. Both V2.1 and V3 defeat this because \texttt{\$pl\_demote} is supplemented into the auth overlay, superseding the ancient PL.}
\label{fig:demotion_evasion}
\end{figure}

\section{Conclusion}
By carefully balancing the supplemental merge, State Resolution V3 provides a mathematically secure consensus algorithm that protects against both historical lockouts and concurrent evasion attacks.
