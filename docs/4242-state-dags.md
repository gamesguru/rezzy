...
events (_auth events_), which make up the part of the room state necessary for the authorisation decision
(_auth state_). Auth events are state events, and defined as all events of types relevant to write access
...
- Concurrent authorisation: If an event affects room state of an honest server, that event is
  authorised by the room state resolved from all events in the state DAG before _or concurrent to_
  the given event.
...
authorisation properties on the other hand describe _how_ we want consistent authorisation to look like.
...
_A room DAG showing the `auth_events` (dashed arrows) for a `m.room.message`.
The `prev_events` path is shown with solid arrows._
...
_A join event with its `auth_events` in blue, along with the invites `auth_events` in red, forming a "chain"._
...
_Bob attempting to evade a ban by referencing in his `auth_events` his join event from earlier._
...
_Bob referencing the power level event when he was a moderator, despite never being joined and a moderator at the same time._
...
_Proposed room DAG with state DAG overlaid in dashed lines._
...
_Both `Msg1` and `Msg2` will reference `prev_state_events: [State3, State4]`, as that is the current
state forward extremities._
...
![sf2](https://github.com/user-attachments/assets/7ff1effa-9797-46ba-bbe5-b4aa01a04fc0)
...
If the `limit` is reached, the sending server should continue to walk back until it has discovered _all_
`prev_state_events`. Care must be taken to ensure _all_ paths to the `m.room.create` event are walked. Consider:
...
_auth events_ subset of state events, thus creating an "auth DAG". However, problems occur when you
...
_A state event references very old `prev_auth_events: [Auth1]` which aren't even on the same fork according to `prev_events: [Auth3]` as the state event in question._
...
_Alice's message should not have `prev_events: [Msg1]` but it does._
...
_`New msg` should be pointing to `State2` since it is pointing to `Msg2` as a `prev_event`, but it chooses not to._
