...
![Architecture diagram](images/4430-1.png)
...
_TODO: how does this work when a local user invites a federated user? We presumably send that over federation but they can never accept it?_
...
[^urlsafe]:
    We want the member key to be url-safe because it frequently appears in URL paths in the client-server API e.g account data,
